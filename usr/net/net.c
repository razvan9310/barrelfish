...
static errval_t _real_send_ip_packet(uint32_t src, uint32_t dst, uint8_t protocol, uint8_t *payload, size_t len) {
    ...
    make_packet(packet, src, dst, protocol, payload, len);
...
errval_t send_packet(uint32_t src, uint32_t dst, uint8_t protocol, uint8_t *payload, size_t len) {
    return _real_send_packet(src, dst, protocol, payload, len); // this should be through an RPC
}
